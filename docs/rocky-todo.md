...
- **보드 생성**: 사이드바 보드 목록 아래 `+ 새 보드`. key 에 공백이나 `#` 이 들어가면
  거절된다 — 그런 key 는 `my repo#1` 같은 해석 불가한 참조를 만들기 때문이고, 이유가
  입력창 아래 그대로 표시된다.
- **섹션 생성**: 보드를 고른 상태에서 quick-add 아래 `+ 섹션`. `전체` 뷰에서는 어느 보드에
  만들지 정할 수 없어 숨는다. 같은 이름이면 새로 만들지 않고 기존 섹션을 쓴다.
