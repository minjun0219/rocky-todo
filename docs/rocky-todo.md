...
상단 바 오른쪽의 글리프 버튼을 누르면 테마가 `◐`(auto) → `●`(어두움) → `○`(밝음) → `◐`
순으로 순환한다. `auto` 는 OS 다크모드 설정을 따르고, 세션 중 OS 설정이 바뀌면 새로고침
없이 그대로 따라온다. `어두움`/`밝음` 을 명시적으로 고르면 OS 설정과 무관하게 그 테마로
고정되고, 선택은 브라우저 `localStorage` 에 저장되어 다음 방문에도 유지된다. 테마는 첫
페인트 전에 인라인 스크립트로 확정되므로 새로고침해도 화면이 번쩍이지 않는다.
