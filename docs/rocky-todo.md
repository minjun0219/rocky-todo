...
떨어진다 — 에러 화면을 띄우지 않는다. board key `api`/`mcp` 도 보드로 만들고 쓸 수 있지만,
데몬의 `/api/*`/`/mcp` 라우트와 겹쳐 URL 로 가리킬 수 없어 주소는 항상 `/` 로 남는다.
...
- **보드 생성**: 사이드바 보드 목록 아래 `+ 새 보드`. key 에 공백이나 `#` 가 들어가면
  ...
  그대로 표시된다. `api`/`mcp` 는 거절되지 않고 만들어지지만, URL 이 `/api/*`/`/mcp` 와
  겹쳐 그 보드의 주소는 항상 `/` 로 남는다.
