...
메모는 `note-3`)가 뜬다. 그 참조를 클릭하면 `/rocky-todo:board rocky-12` 슬래시 커맨드가
클립보드로 복사된다 — 세션에 그대로 붙여넣으면 그 항목을 맡아 착수한다. `navigator.clipboard` 는
...
옛 표기(`rocky#12` / `#12`)도 입력으로는 계속 받는다. `ls` 출력의 `rocky-12` 를 그대로 다음
명령의 REF 로 쓰면 된다. 랜덤 id 는 여전히 기본 키이고 `show` 상세 출력의
`id:` 줄에서 볼 수 있다. 보드 미소속 글로벌 메모는 번호가 `note-3` 으로 표시되고,
그 번호를 보드 번호와 구분해 조회하려면 `note show|edit|append|archive`/`history` 에
`--global` 을 붙인다. `note` 는 예약된 접두사라 보드 이름으로 새로 쓸 수 없다(기존에
있던 `note` 보드는 계속 동작하되 그 안의 항목은 raw id 로만 가리킬 수 있다). todo 와 메모는
