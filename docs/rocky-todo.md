...
떨어진다 — 에러 화면을 띄우지 않는다. board key `api`/`mcp` 는 데몬의 `/api/*`/`/mcp` 라우트와
겹쳐 새로 만들 수 없다(검증 전에 만들어진 그런 보드는 계속 조회되지만 주소는 `/` 로 남는다).
...
- **보드 생성**: 사이드바 보드 목록 아래 `+ 새 보드`. key 에 공백이나 `#`, 혹은 데몬 라우트와
  겹치는 `api`/`mcp` 가 들어가면 거절된다 — 공백·`#` 는 `my repo#1` 같은 해석 불가한 참조를
  만들기 때문이고, `api`/`mcp` 는 URL 이 `/api/*`/`/mcp` 와 겹치기 때문이다. 이유가 입력창
  아래 그대로 표시된다.
