...
:root[data-theme='light'] {
  ...
  --muted: #6b5c4a;       /*  5.45 */
  --faint: #756753;       /*  4.64 */
  --warm: #6b4600;        /*  7.09 — dim 이 설 자리를 남기려 깊게 잡았다 */
  --warm-dim: #8b6019;    /*  4.69 */
  --cool: #195564;        /*  7.03 — warm 과 같은 이유로 깊다 */
  --cool-dim: #52828d;    /*  3.59 — 테두리 전용이라 3.0 기준 */
  --p1: #b3311c;          /*  5.26 */
  --p2: #6b4600;          /*  7.09 — --warm 과 같은 값(앰버 = p2) */
  --p3: #6f6215;          /*  5.17 */
  --ok: #3f6b26;          /*  5.31 */
  --handoff: #1f5aa8;     /*  5.75 */
...
`--warm` / `--cool` 은 기준(4.5)만 겨우 넘기는 값에서 한 단계 더 깊게 내렸다. 두 색은
각각 `--warm-dim` / `--cool-dim` 과 짝을 이루는데, 본색이 얕으면 dim 이 설 자리가 없어
둘이 같은 색으로 읽힌다. `--p2` 는 앰버라는 의미를 `--warm` 과 공유하므로 같은 값이다.
