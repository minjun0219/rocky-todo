...
`"tailscale-serve"` 는 **의사 표시일 뿐 자동 설정이 아니다** — 데몬은 `tailscale serve` 를
직접 건드리지 않으니, 실제로 열려면 `rocky-todo tailscale on` 을 한 번 실행해야 한다.
443 의 `/` 매핑은 머신 전체가 공유하는 자원이라, 데몬이 기동할 때마다 잡으면 포트가 다른
다른 인스턴스(개발 워킹트리 등)의 노출을 덮어쓰기 때문이다.
